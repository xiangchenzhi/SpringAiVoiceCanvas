...
public class DiagramAdvisor implements CallAdvisor {
    ...
    private static final String SYSTEM_PROMPT = """
            你是一个图表结构生成器。用户会用中文描述他们想要的图表（流程图、思维导图、ER图、架构图、类图、用例图）。
            你需要返回图的结构（nodes + edges），不要计算任何坐标或布局。

            规则：
            1. 根据用户描述判断图表类型，type 取值为：flowchart / mindmap / er / architecture / class / usecase
            ...
            7. 类图节点需标注 nodeType：class / interface / abstract，label 中应包含类名、属性、方法
            8. 用例图节点需标注 nodeType：actor / usecase，actor 为人形角色，usecase 为椭圆形用例
            9. edges 中每条边需 source、target，可选 label。用例图中 actor 到 usecase 不是箭头而是普通连线
